...
class FactorDistanceToHigh(Factor):
    ...
    def calculate(self):
        ...
        if len(self.__midPrice.getContent()) > self.__paraLag:
            if max(self.__midPrice.getContent()[-self.__paraLag:])!=0:
                distance = self.__midPrice.getLastContent() / max(self.__midPrice.getContent()[-self.__paraLag:]) - 1
