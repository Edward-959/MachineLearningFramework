...
class FactorAccumBuyPower(Factor):
    ...
    def calculate(self):
        ...
        if len(self.__historyVolume.getContent()) <= self.__paraMAVolumeLag:
            FactorValue = float(len(self.__historyVolume.getContent()) * accBidAmount) / sum(self.__historyVolume.getContent())
        else:
            FactorValue = float(self.__paraMAVolumeLag * accBidAmount) / sum(self.__historyVolume.getContent()[-self.__paraMAVolumeLag:])
