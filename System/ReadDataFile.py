...
def getData(stockCode, startDateTime, endDateTime, exchangeTradingDayList=None, timeMode=1):
    ...
    startTime11digit = startDateTime.hour * 10000000 + startDateTime.minute*100000+ startDateTime.second * 1000
    endTime11digit = endDateTime.hour * 10000000 + endDateTime.minute*100000 + endDateTime.second * 1000
    ...
    for i in range(yearMonthList.__len__()):
        if (stockCode[0] == "0" and stockCode[-1] == "H") or (stockCode[:2] == "39" and stockCode[-1] == "Z"):  # 如果是指数，则在指数文件夹下读取文档
            matFilePath = 'E:\\MatData\\IndexTickData\\' + stockCode + '\\TickInfo_' + stockCode + '_' + yearMonthList[i] + '.mat'
        elif (stockCode[0] == "6" and stockCode[-1] == "H") or (stockCode[0] == "0" and stockCode[-1] == "Z") or (stockCode[0] == "3" and stockCode[-1] == "Z"):
            matFilePath = 'E:\\MatData\\StockTickData\\' + stockCode + '\\TickInfo_' + stockCode + '_' + yearMonthList[i] + '.mat'
        elif stockCode[:5] == "50ETF":
            matFilePath = 'C:\\Users\\USER\\Desktop\\MachineLearningBackTestFrameWork\\50ETFOption_mainContractData\\'+ stockCode[-14:]+'\\'+ stockCode + '_' + yearMonthList[i].strftime('%Y%m%d') + '.mat'
        try:
    ...
    if timeMode == 1:
        timeFilter = (startTime11digit <= StockData['Time']) & (StockData['Time'] <= endTime11digit) &\
