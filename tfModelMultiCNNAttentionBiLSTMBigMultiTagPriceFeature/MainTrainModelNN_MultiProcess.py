...
from tfModelMultiCNNAttentionBiLSTMBig.ModelMultiCNNAttentionBiLSTM import ModelMultiCNNAttentionBiLSTM
...
def multiProcessFunction(paraInProcess, paraFixed, absolutePath, backTestUnderlying, modelName):
    ...
    for para in paraInProcess:
        ...
        # 其余不变
        signalEvaluate = SignalEvaluate(signalDataSet, signalTagPara, mockTradePara, "SignalExecutorMultiTag", absolutePath,
...
def main():
    ...
    # 设置是否需要加载因子去计算预测值
    mode = False
    absolutePath = "/app/data/054703/ChiBinhan_SelectMultiTag16/"
    factorAddress = 'output/20180413cbhAndzhgNew/'
    tagName = ["2minMaxMin"]
    # tagName = ["2minRD"]
    # tagName = ["1minMaxMin","1minRD"]
    conv_len = 3
    sliceLagMat = [150]
    predictRollingWindowMat = [0.2]
    processNum = 6
    paraFixed = {'riskRatio': -0.001, 'maxLose': -0.01, 'maxTurnoverPerOrder': 5000000, 'maAmountRate': 2}
    paraMat = [[0.0008, 0.0009, 0.001, 0.0011, 0.0012, 0.0013],
               [50000000]]
    # 设置需要训练的因子
                ...
                for k in predictRollingWindowMat:
                    ...
                    if mode:
                        modelManagement = ModelManagement(trainRollingWindow=1 - predictRollingWindow,
                                                          ...
                                                          backTestUnderlying=backTestUnderlying, tagName=tagName[0])
                    # 设置模型参数
                    ...
                    if not os.path.exists(absolutePath + 'ModelSignalDataSet/' + backTestUnderlying + '_' + modelName + '/Data.pickle'):
                        ...
                        startTime = dt.datetime.fromtimestamp(signalDataSet.outputByUnderlying()[0]['outSampleSubTag'][0][tagName[0]].startTimeStamp)
                        endTime = dt.datetime.fromtimestamp(signalDataSet.outputByUnderlying()[0]['outSampleSubTag'][-1][tagName[0]].startTimeStamp)
                    ...
                    signalEvaluate = SignalEvaluate(signalDataSet, signalTagPara, mockTradePara,
                                                    "SignalExecutorMultiTag", absolutePath, tickData,
                                                    transactionData)
                    # 执行
                    ...
                    print('initAmount: ' + str(analysisMat[bestLine][1]))
                    # del signalEvaluate
                    del tickData
                    del transactionData
                    gc.collect()
