    ...
    def train(self, rollingData, outputDict):  # 数据集都是np.array格式
        ...
        with tf.Session() as sess:
            ...
            if not os.path.exists(
                    absolutePath + 'ModelSaved/' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder'):
                ...
                numVariable = [0]
                y_regression, rnn_keepProb = deepnn(x, numVariable, image_len=sliceLag, factor_num=numFactor,
                                                    conv1_len=conv_len,
                ...
                for i in range(runStep):
                    ...
                    if i % checkPeriod == 0:
                        ...
                        if train_TriggerTimes != 0:
                            train_winRate = (sum(label[predict > triggerRatio] > 0.001) + sum(
                                label[predict < -triggerRatio] < -0.001)) / train_TriggerTimes
                            train_AveReturn = (sum(label[predict > triggerRatio]) - sum(
                                label[predict < -triggerRatio])) / train_TriggerTimes
                        else:
                            train_winRate = 0
                            train_AveReturn = 0
                        ...
                        if valid_TriggerTimes != 0:
                            valid_winRate = (sum(label[predict > triggerRatio] > 0.001) + sum(
                                label[predict < -triggerRatio] < -0.001)) / valid_TriggerTimes
                            valid_AveReturn = (sum(label[predict > triggerRatio]) - sum(
                                label[predict < -triggerRatio])) / valid_TriggerTimes
                        else:
                            valid_winRate = 0
                            valid_AveReturn = 0

                        print(
                            'step %d, training loss %.6f, winRate %.2f, times %d, train AveReturn %.6f, epochs %d; valid loss %.6f, winRate %.2f, times %d, valid AveReturn %.6f' % (
                            i, train_loss, train_winRate, train_TriggerTimes, train_AveReturn,
                            trainDeepNNDataSet.epochs_completed, valid_loss, valid_winRate, valid_TriggerTimes,
                            valid_AveReturn))
                        ...
                        if valid_loss <= min(valid_lossSeries):
                            ...
                            if i == 0:
                                os.mkdir(
                                    absolutePath + 'ModelSaved/tmp' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder')
                                shutil.move(
                                    absolutePath + 'ModelSaved/' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder' + "/saved_model.pb",
                                    absolutePath + 'ModelSaved/tmp' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder' + "/saved_model.pb")
                            print('Model Saved')
                            with open(
                                    absolutePath + 'ModelSaved/' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder' + "/ModelSet.json",
                                    "w") as f:
                                modelSet = {'mean': scale_mean.tolist(), 'std': scale_std.tolist(),
                                            'ModelInput': self.modelInput, 'WindowSize': sliceLag}
                        ...
                        if trainDeepNNDataSet.epochs_completed == epochsStop:
                            with open(
                                    absolutePath + 'ModelSaved/' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder' + "/Loss.json",
                                    "w") as f:
        ...
        with tf.Session() as sess:
            ...
            with open(
                    absolutePath + 'ModelSaved/' + self.backTestUnderlying + '_' + self.name + '_SavedModelBuilder' + "/ModelSet.json",
                    "w") as f:
                modelSet = {'mean': scale_mean.tolist(), 'std': scale_std.tolist(), 'ModelInput': self.modelInput,
                            'WindowSize': sliceLag}
                json.dump(modelSet, f)
            ...
            if TriggerTimes != 0:
                winRate = (sum(inSampleLabel[inSamplePredict > triggerRatio] > 0.001) + sum(
                    inSampleLabel[inSamplePredict < -triggerRatio] < -0.001)) / TriggerTimes
                allTrain_AveReturn = (sum(inSampleLabel[inSamplePredict > triggerRatio]) - sum(
                    inSampleLabel[inSamplePredict < -triggerRatio])) / TriggerTimes
            else:
                winRate = 0
                allTrain_AveReturn = 0
            print('allTrain loss %.6f, winRate %.2f, times %d, allTrain AveReturn %.6f' % (
            allTrain_loss, winRate, TriggerTimes, allTrain_AveReturn))
            ...
            if TriggerTimes != 0:
                winRate = (sum(outSampleLabel[outSamplePredict > triggerRatio] > 0.001) + sum(
                    outSampleLabel[outSamplePredict < -triggerRatio] < -0.001)) / TriggerTimes
                test_AveReturn = (sum(outSampleLabel[outSamplePredict > triggerRatio]) - sum(
                    outSampleLabel[outSamplePredict < -triggerRatio])) / TriggerTimes
            else:
                winRate = 0
                test_AveReturn = 0
            print('test loss %.6f, winRate %.2f, times %d, test AveReturn %.6f' % (
            test_loss, winRate, TriggerTimes, test_AveReturn))
        ...
        outputDict.update(
            self.evaluateModel(inSamplePredict, inSampleLabel, "inSample", self.paraModel["evaluateModel"]))
        outputDict.update(
            self.evaluateModel(outSamplePredict, outSampleLabel, "outSample", self.paraModel["evaluateModel"]))


def deepnn(x, numVariable, image_len, factor_num, conv1_len, conv1_num, pool1_len, conv2_len, conv2_num, pool2_len,
    ...
    with tf.name_scope('Inception1'):
        ...
        h_Inception1_Branch2 = tf.nn.relu(
            conv2d(h_Inception1_Branch2, W_Inception1_Branch2, [1, 1], 'SAME') + b_Inception1_Branch2)
        ...
        h_Inception1_Branch3 = tf.nn.relu(
            conv2d(h_Inception1_Branch3, W_Inception1_Branch3, [1, 1], 'SAME') + b_Inception1_Branch3)
        ...
        h_Inception1_Branch4 = tf.nn.relu(
            conv2d(h_Inception1_Branch4, W_Inception1_Branch4, [1, 1], 'SAME') + b_Inception1_Branch4)
    ...
    timestep_size = int(
        math.ceil(math.ceil(math.ceil((image_len - conv1_len + 1) / pool1_len) / pool2_len) / pool3_len))
    ...
    with tf.name_scope('regression'):
        W_fc = weight_variable([rnn_hiddenSize, 1], regularizationRate, numVariable)
        b_fc = bias_variable([1], numVariable)

        y_regression = tf.matmul(h_state, W_fc) + b_fc
