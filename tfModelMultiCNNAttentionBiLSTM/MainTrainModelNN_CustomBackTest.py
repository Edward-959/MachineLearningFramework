...
def main():
    ...
    for i in range(0, 14):
        ...
        if not os.path.exists(factorAddress + backTestUnderlying + '.pickle'):
            ...
            factorName = []
            if mode:
                outputFactor, outputSubTag, tradingUnderlyingCode, factorName = DumpLoad.loadOutput(factorAddress,
                                                                                                    backTestUnderlying)
            # 设置需要训练的因子
                ...
                for k in predictRollingWindowMat:
                    ...
                    if not os.path.exists(
                        ...
                        startTime = dt.datetime.fromtimestamp(
                            signalDataSet.outputByUnderlying()[0]['outSampleSubTag'][0][tagName].startTimeStamp)
                        endTime = dt.datetime.fromtimestamp(
                            signalDataSet.outputByUnderlying()[0]['outSampleSubTag'][-1][tagName].startTimeStamp)
